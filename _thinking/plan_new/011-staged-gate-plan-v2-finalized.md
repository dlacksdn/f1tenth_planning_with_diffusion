...
## 8. 추가 검토 방향 (append, 2026-06-20) — 완주를 value에 반영(γ) + 고품질 데이터 수집

> 010 §4("value는 완주를 점수로 못 준다")에 대한 후속 토론. 사용자 제기 = **(1) γ(discount)를 더 키운다
> (2) 고품질 데이터를 더 모은다.** 둘 다 stretch(<56s)를 살리는 *보강 방향*으로 검토. 확정 아닌 옵션이나,
> §2.1(stretch 병목=재료의 질)을 정면으로 공략하므로 기록해 둔다.

### 8.1 완주 반영 = γ를 키운다 (value만 재학습 — diffusion 무관)
- 1차 소스: value 타깃 = RTG = Σ γ^k·r (sequence.py:143-148, `rewards[start:]` = start부터 **ep 끝까지**).
  γ는 **config 파라미터일 뿐**(`values.discount`=config:78, 로드경로 `plan.discount`=config:143 — D5처럼 둘
  다 바꿔야 로드 일치). **diffusion은 reward를 안 보므로 100% 재사용, value만 ~5-10h 재학습.**
- γ별 "1400 step 앞 완주 보너스(+100)" 기여 + cap10 완주 RTG(출발점) 분해 (산술):

  | γ | 유효 지평 | 1400 step 앞 완주보너스 기여 | cap10 RTG ≈ progress + 완주 |
  |---|---|---|---|
  | **0.99**(현재) | ~100 step(2초) | 0.99¹⁴⁰⁰×100 ≈ **0.0001**(안 보임) | 20 + 0 = 20 |
  | 0.997 | ~330 step | ~1.5 | 57 + 4 ≈ 61 |
  | **0.999** | ~1000 step(20초) | 0.999¹⁴⁰⁰×100 ≈ **24.7**(보임!) | 188 + 30 ≈ 218 |

  → **γ=0.999면 완주 보너스가 0.0001→24.7로 살아난다.** "다른 방도"가 바로 이것(config 한 줄 + value 재학습).
- 트레이드오프(정직): progress 누적도 같이 커져(20→188) value 스케일·분산↑(`normed=True`가 [-1,1]로
  압축하나 해상도 저하), credit assignment 흐려짐, 충돌 페널티 −10은 여전히 작아 **페널티 동반 조정**이
  필요할 수 있음. → γ 단독이 아니라 γ + 벌점의 *조합 스윕*이 맞다.

### 8.2 고품질 데이터 수집 — value는 prior가 만든 것 중에서만 고른다
- ★ γ는 value의 **"고르는 능력"**만 키운다. value는 **prior(=데이터)가 생성 가능한 plan 중에서만** 고른다.
  prior가 "빠르면서 2랩 완주"를 못 만들면 γ를 고쳐도 고를 대상이 없다.
- 현재 데이터 최속 완주 = **cap10 56s**(56s보다 빠른 2랩완주 **0개**, 014 실측). 고속 재료(cap15/20)는
  단일랩 한계주행이라 2랩 못 버팀(§2.1). → stretch엔 **"56s보다 빠르면서 2랩 완주하는" 재료가 필수**이고,
  γ로도 짜내기로도 안 나온다 → **새 수집이 정공법**:
  - RL_project에서 **중간 캡 정책(cap-12/13 등)** 학습 → 완주 rollout(48~52s급 안전 완주 재료).
  - no-expert 제약 안 깸(캡 정책이지 expert 아님). ★**교수 기준과 일치** — "빠른 재료가 데이터에 실제
    포함돼야 stitching 가능"(교수 확정, 메모리). 즉 고품질 수집은 우회로가 아니라 정공법.
  - 비용 ~1-2h/tier(완주율 높아 효율적).
- → **011 §4.2 "재수집 후순위"를 정정**: stretch를 진짜 노린다면 고품질 수집은 *필수 경로*다(후순위 아님).
  단 순서는 여전히 Track A 먼저(아래 8.4).

### 8.3 긴장 (정직)
빠른 완주를 데이터에 넣을수록 Diffuser가 **BC 모방만으로** 그 속도를 냄 → "개선"(데이터 최속 완주 초과)
정의가 더 빡빡해짐. 그러나 정성평가라 "빠른 재료들 + stitching으로 안전하게 *더 빠른* 완주"면 정당하고,
BC 모방 자체도 baseline 107s 대비 큰 개선이다.

### 8.4 위치·순서 (두 레버는 경쟁이 아니라 상호보완)
| 레버 | 무엇을 고치나 | 비용 | 언제 |
|---|---|---|---|
| **γ 키우기**(0.99→0.997/0.999) | value가 완주·먼 충돌을 **보게** | value만 ~5-10h(diffusion 재사용) | 즉시 — Track B에 **γ 스윕** 추가(B1 벌점과 *조합*) |
| **고품질 데이터 수집**(중간 캡 빠른 완주) | prior가 빠른 완주를 **만들게** | RL 학습+rollout ~1-2h/tier | **Track A(안전 복원) 확정 후** stretch 본격 투자 |
- 무비용 선검증(011 §3.2 B0 확장): **γ별로** "완주 plan vs 충돌 plan"의 RTG가 실제로 갈리는지 기존
  데이터로 재계산(읽기만). 갈리면 γ 스윕이 실효, 안 갈리면 고품질 데이터 없인 stretch 불가 확정.
- 핵심: γ(value 능력) + 고품질 데이터(prior 재료)가 **함께** 있어야 stretch가 열린다. 어느 하나만으론 부족.
